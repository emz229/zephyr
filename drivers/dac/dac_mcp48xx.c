...
struct mcp48xx_config {
	...
	uint32_t pending_value[MCP48XX_MAX_CHANNEL];
};
...
static int mcp48xx_write_direct(const struct device *dev, uint8_t channel, uint32_t value)
{
...
static int mcp48xx_write_buffer(const struct device *dev)
{
	const struct mcp48xx_config *config = (struct mcp48xx_config *)dev->config;
        uint8_t tx_data[2];
        int ret;
	...
	for (int i = 0; i < config->nchannels; i++) {
		if (config->resolution == 8) {
			dac_value = config->pending_value[i] << 4;
		}
		else {
			if (config->resolution == 10) {
				dac_value = config->pending_value[i] << 2;
			}
		}

		tx_data[0] = 0;
		tx_data[0] = i << 7; /* Bit 15 is A / B channel */
		tx_data[0] = tx_data[0] | (config->gain[i] << 5); /* Bit 13 is Gain */
		tx_data[0] = tx_data[0] | (config->power_down[i] << 4); /* Bit 12 is SHDN */
		tx_data[0] = tx_data[0] | ((dac_value >> 8) & 0x0000000F); /* Bit 11-8 is upper nibble */
		tx_data[1] = 0;
		tx_data[1] = dac_value;

		LOG_DBG("tx[0] = %x\ntx[1] = %x", tx_data[0], tx_data[1]);
...
static int mcp48xx_write_value(const struct device *dev, uint8_t channel, uint32_t value)
{
	...
	if (channel == DAC_WRITE_ALL_CH) {
	        /* latch GPIO configured */
		if (config->gpio_latch.port != NULL) {
			/* write pending data to DAC HW */
			ret = mcp48xx_write_buffer(dev);
			if (ret != 0) {
				LOG_ERR("failed to write buffer: %i", ret);
				return ret;
			}
			k_usleep(1);
			/* apply actual DAC output */
			ret = dac_mcp48xx_latch(dev);
			if (ret != 0) {
				LOG_ERR("failed to apply GPIO latch: %i", ret);
				return ret;
			}
		}
	}
	/* no latch GPIO configured */
	else {
		/* write directly to DAC HW */
		mcp48xx_write_direct(dev, channel, value);
	}

	return 0;
}

static int mcp48xx_update_value(const struct device *dev, uint8_t channel, uint32_t value)
{
	struct mcp48xx_config *config = (struct mcp48xx_config *)dev->config;
	...
	/* Update SW DAC buffer value */
	config->pending_value[channel] = value;
	return 0;
}
...
static const struct mcp48xx_chip_api mcp48xx_driver_api = {
	.dac_api = {
		...
		.update_value = mcp48xx_update_value,
	},
